...
;; fn loi > image
;; takes loi of combines all using ??
(check-expect (combine-all beside (list (circle 10 "outline" "red") (circle 20 "outline" "blue") (circle 10 "outline" "yellow")))
              (beside (circle 10 "outline" "red") (circle 20 "outline" "blue") (circle 10 "outline" "yellow")))
(check-expect (combine-all above (list (circle 30 "outline" "black") (circle 50 "outline" "black") (circle 70 "outline" "black")))
              (above (circle 30 "outline" "black") (circle 50 "outline" "black") (circle 70 "outline" "black")))

(define (combine-all c loi)
  (cond [(empty? loi) empty-image]
        [else
         (c (first loi)
            (combine-all c (rest loi)))]))
...
;; (String Y -> Y) (X Y -> Y) Y Region -> Y
;; abstract fold function

(define (abs-fold-region c1 c2 b r)
  (local [(define (fn-for-region r)                       ;X
          ...
          (define (fn-for-lor lor)                        ;Y
            (cond [(empty? lor) b]                        ;b
                  [else
                   (c2 (fn-for-region (first lor))        ;in:c2 ;out:X
